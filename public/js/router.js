(function() {
  ...
  const MODAL_ICONS = {
    ...
    archive: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="5" rx="1"/><path d="M4 9v10a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V9"/><path d="M10 13h4"/></svg>'
  };
    ...
    open(opts) {
      ...
      m.innerHTML =
        '<div class="modal__header">' +
          ...
          '</button>' +
        '</div>' +
        (opts.context ? '<div class="modal__context"></div>' : '') +
      ...
      if (opts.context) {
        const ctxEl = m.querySelector('.modal__context');
        if (typeof opts.context === 'string') {
          ctxEl.textContent = opts.context;
        } else if (opts.context instanceof HTMLElement) {
          ctxEl.appendChild(opts.context);
        }
      }
